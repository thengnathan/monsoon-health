...
export default function ScreeningCaseDetailPage() {
    ...
    // Status update
    const [statusForm, setStatusForm] = useState({ status: '', next_action_date: '' });
    const handleStatusUpdate = async (e) => {
        e.preventDefault();
        try {
            await api.updateScreeningCase(id, statusForm);
    ...
    // Screen fail
    const [failForm, setFailForm] = useState({ status: 'SCREEN_FAILED', fail_reason_id: '', fail_reason_text: '', what_would_change_text: '', revisit_date: '' });
    const handleScreenFail = async (e) => {
        e.preventDefault();
        try {
            const payload = { ...failForm };
    ...
    // Enrollment
    const [enrollForm, setEnrollForm] = useState({ enrollment_date: new Date().toISOString().split('T')[0] });
    const handleEnroll = async (e) => {
        e.preventDefault();
        try {
            const result = await api.enrollPatient(id, enrollForm);
    ...
    // Visit status update
    const handleVisitStatusChange = async (visitId, status, actualDate) => {
        try {
            const data = { status };
            ...
            api.getCaseVisits(id).then(setVisits);
        } catch (err) { addToast(err.message, 'error'); }
    };

    // Pending items
    const [pendingForm, setPendingForm] = useState({ type: 'LAB', name: '', due_date: '' });
    ...
    const togglePendingItem = async (item) => {
    ...
    const visitStatusIcons = { SCHEDULED: '○', COMPLETED: '●', MISSED: '✕', CANCELLED: '—' };
    const visitStatusColors = { SCHEDULED: 'var(--accent)', COMPLETED: 'var(--success)', MISSED: 'var(--error)', CANCELLED: 'var(--text-tertiary)' };
                        ...
                        <div className="detail-section">
                            ...
                            <div style={{ display: 'flex', gap: 'var(--space-3)', flexWrap: 'wrap' }}>
                                {sc.trial_signal_rules.map(rule => {
                                    const opLabels = { GTE: '≥', LTE: '≤', EQ: '=', IN: 'in' };
                                    return (
