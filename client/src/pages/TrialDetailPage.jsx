...
export default function TrialDetailPage() {
    ...
    const [criteriaForm, setCriteriaForm] = useState({ inclusion_criteria: '', exclusion_criteria: '' });
    ...
    const handleSaveCriteria = async () => {
        try {
            await api.updateTrial(id, criteriaForm);
    ...
    // Visit templates
    const [visitForm, setVisitForm] = useState({ visit_name: '', day_offset: 0, window_before: 3, window_after: 3, reminder_days_before: 3, sort_order: 0 });

    const handleAddVisit = async (e) => {
        e.preventDefault();
        try {
            await api.createVisitTemplate(id, { ...visitForm, day_offset: parseInt(visitForm.day_offset), sort_order: parseInt(visitForm.sort_order) });
            addToast('Visit added', 'success');
            setShowVisitModal(false);
            setVisitForm({ visit_name: '', day_offset: 0, window_before: 3, window_after: 3, reminder_days_before: 3, sort_order: 0 });
            loadTrial();
        } catch (err) { addToast(err.message, 'error'); }
    };

    const handleDeleteVisit = async (tmplId) => {
        try {
    ...
    // Signal rules
    const [signalForm, setSignalForm] = useState({ signal_type_id: '', operator: 'GTE', threshold_number: '', unit: '' });

    const handleAddSignal = async (e) => {
        e.preventDefault();
        try {
            const payload = { ...signalForm };
            if (payload.threshold_number) payload.threshold_number = parseFloat(payload.threshold_number);
            await api.createTrialRule(id, payload);
            addToast('Signal rule added', 'success');
            setShowSignalModal(false);
            setSignalForm({ signal_type_id: '', operator: 'GTE', threshold_number: '', unit: '' });
            loadTrial();
        } catch (err) { addToast(err.message, 'error'); }
    };

    const handleDeleteSignal = async (ruleId) => {
        try {
    ...
    const statusColors = { ACTIVE: 'var(--status-enrolled)', PAUSED: 'var(--status-in-review)', CLOSED: 'var(--text-tertiary)' };
    const operatorLabels = { GTE: '≥', LTE: '≤', EQ: '=', IN: 'in' };
                        ...
                        {trial.protocol ? (
                            <div className="card" style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-4)' }}>
                                ...
                                <div style={{ display: 'flex', gap: 'var(--space-2)' }}>
                                    <a href={api.getProtocolUrl(id)} target="_blank" rel="noopener" className="btn btn-sm btn-secondary">View PDF</a>
