...
export default function PatientDetailPage() {
    ...
    const [sigForm, setSigForm] = useState({ signal_type_id: '', value: '', collected_at: new Date().toISOString().split('T')[0], source: '' });
    const [caseForm, setCaseForm] = useState({ trial_id: '', status: 'NEW' });

    const handleAddSignal = async (e) => {
        e.preventDefault();
        try {
            const result = await api.addPatientSignal(id, sigForm);
