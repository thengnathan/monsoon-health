...
router.get('/', async (req: Request, res: Response) => {
    ...
    const { screening_case_id, status } = req.query as { screening_case_id?: string; status?: string };

    let sql = 'SELECT * FROM pending_items WHERE site_id = $1';
    const params: unknown[] = [req.user.site_id];
    let p = 1;

    if (screening_case_id) { sql += ` AND screening_case_id = $${++p}`; params.push(screening_case_id); }
...
router.post('/', async (req: Request, res: Response) => {
    const db = req.app.locals.db;
    const id = uuidv4();
    const { screening_case_id, type, name, due_date } = req.body as {
        screening_case_id?: string; type?: string; name?: string; due_date?: string;
    };

    if (!screening_case_id || !type || !name) {
        res.status(400).json({ error: 'screening_case_id, type, and name are required' }); return;
    }
...
router.patch('/:id', async (req: Request, res: Response) => {
    ...
    if (!existing) { res.status(404).json({ error: 'Pending item not found' }); return; }
...
router.delete('/:id', async (req: Request, res: Response) => {
    ...
    if (result.rowCount === 0) { res.status(404).json({ error: 'Pending item not found' }); return; }
