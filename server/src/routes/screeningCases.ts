...
router.get('/', async (req: Request, res: Response) => {
    ...
    const { status, trial_id, assigned_user_id, patient_id, revisit_due, limit = '50', offset = '0' } = req.query as Record<string, string>;
  ...
  `;
    ...
    if (cases.length > 0) {
        ...
        const countMap: Record<string, { total: string; open_count: string }> = {};
        pendingCounts.forEach((c: { screening_case_id: string; total: string; open_count: string }) => {
            countMap[c.screening_case_id] = c;
        });
        cases.forEach((c: Record<string, unknown>) => {
            c.pending_items_total = Number(countMap[c.id as string]?.total || 0);
            c.pending_items_open = Number(countMap[c.id as string]?.open_count || 0);
        });
    }
...
router.post('/', async (req: Request, res: Response) => {
    const db = req.app.locals.db;
    const id = uuidv4();
    const { patient_id, trial_id, assigned_user_id, status = 'NEW' } = req.body as {
        patient_id?: string; trial_id?: string; assigned_user_id?: string; status?: string;
    };

    if (!patient_id || !trial_id) {
        res.status(400).json({ error: 'patient_id and trial_id are required' }); return;
    }
    ...
    if (existingActive) {
        res.status(409).json({ error: 'An active screening case already exists for this patient-trial combination', existing_case_id: existingActive.id }); return;
    }
...
router.get('/:id', async (req: Request, res: Response) => {
    ...
    if (!sc) { res.status(404).json({ error: 'Screening case not found' }); return; }
...
router.patch('/:id', async (req: Request, res: Response) => {
    ...
    if (!existing) { res.status(404).json({ error: 'Screening case not found' }); return; }
...
router.post('/:id/assign', async (req: Request, res: Response) => {
    ...
    if (result.rowCount === 0) { res.status(404).json({ error: 'Screening case not found' }); return; }
