...
async function scanRevisitDue(db: Pool): Promise<void> {
    try {
            ...
            try {
                await db.query(
                    `INSERT INTO notification_events (id, site_id, type, patient_id, screening_case_id, payload, dedup_key)
                     VALUES ($1, $2, 'REVISIT_DUE', $3, $4, $5, $6)`,
...
async function processNotifications(db: Pool): Promise<void> {
    try {
        const { rows: unprocessed } = await db.query(
        ...
        for (const event of unprocessed) {
            ...
            if (event.screening_case_id) {
                const sc = (await db.query(
                    'SELECT assigned_user_id FROM screening_cases WHERE id = $1',
                    [event.screening_case_id]
                )).rows[0] as { assigned_user_id: string | null } | undefined;
            ...
            if (recipients.length === 0) {
                const { rows } = await db.query(
                    `SELECT id, email, name FROM users WHERE site_id = $1 AND is_active = true AND role IN ('CRC', 'MANAGER')`,
                    [event.site_id]
                );
                recipients = rows;
            }
            ...
            for (const recipient of recipients) {
                await db.query(
                    ...
                    [uuidv4(), event.site_id, recipient.id, event.id, recipient.email, subject, body]
                );
            }

            await db.query(
                `UPDATE notification_events SET processed_at = NOW() WHERE id = $1`,
                [event.id]
            );
        }
...
function generateEmailContent(type: string, payload: Record<string, unknown>): { subject: string; body: string } {
    switch (type) {
        case 'REVISIT_DUE':
            return {
...
async function scanVisitReminders(db: Pool): Promise<void> {
    try {
        ...
        for (const visit of visits) {
            ...
            try {
                await db.query(
                    `INSERT INTO notification_events (id, site_id, type, patient_id, screening_case_id, payload, dedup_key)
                     VALUES ($1, $2, 'VISIT_REMINDER', $3, $4, $5, $6)`,
                ...
                await db.query(
                    'UPDATE patient_visits SET reminder_sent = true WHERE id = $1',
                    [visit.id]
                );
            } catch (_e) {
                // Dedup — already created
