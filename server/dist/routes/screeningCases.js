...
router.get('/', async (req, res) => {
  ...
  `;
    ...
    if (trial_id) {
        sql += ` AND sc.trial_id = $${++p}`;
        params.push(trial_id);
    }
    if (assigned_user_id) {
        sql += ` AND sc.assigned_user_id = $${++p}`;
        params.push(assigned_user_id);
    }
    if (patient_id) {
        sql += ` AND sc.patient_id = $${++p}`;
    ...
    if (cases.length > 0) {
        ...
        const { rows: pendingCounts } = await db.query(`SELECT screening_case_id,
               COUNT(*) as total,
               SUM(CASE WHEN status = 'OPEN' THEN 1 ELSE 0 END) as open_count
             FROM pending_items
             WHERE screening_case_id = ANY($1)
             GROUP BY screening_case_id`, [caseIds]);
        const countMap = {};
        pendingCounts.forEach((c) => {
            countMap[c.screening_case_id] = c;
        });
        cases.forEach((c) => {
...
router.post('/', async (req, res) => {
    ...
    if (!patient_id || !trial_id) {
        res.status(400).json({ error: 'patient_id and trial_id are required' });
        return;
    }
    ...
    const existingActive = (await db.query(`SELECT id FROM screening_cases WHERE patient_id = $1 AND trial_id = $2 AND site_id = $3
         AND status NOT IN ('SCREEN_FAILED', 'DECLINED', 'LOST_TO_FOLLOWUP', 'ENROLLED')`, [patient_id, trial_id, req.user.site_id])).rows[0];
    if (existingActive) {
        res.status(409).json({ error: 'An active screening case already exists for this patient-trial combination', existing_case_id: existingActive.id });
        return;
    }
    await db.query(`INSERT INTO screening_cases (id, site_id, patient_id, trial_id, assigned_user_id, status) VALUES ($1, $2, $3, $4, $5, $6)`, [id, req.user.site_id, patient_id, trial_id, assigned_user_id || req.user.id, status]);
    await (0, auth_1.auditLog)(db, { siteId: req.user.site_id, userId: req.user.id, entityType: 'screening_case', entityId: id, action: 'CREATE', diff: req.body });
    const newCase = (await db.query(`SELECT sc.*, p.first_name, p.last_name, t.name as trial_name
         FROM screening_cases sc
         JOIN patients p ON sc.patient_id = p.id
         JOIN trials t ON sc.trial_id = t.id
         WHERE sc.id = $1`, [id])).rows[0];
    res.status(201).json(newCase);
});
router.get('/:id', async (req, res) => {
    const db = req.app.locals.db;
    const sc = (await db.query(`SELECT sc.*,
    ...
    const [pendingItems, signals, rules] = await Promise.all([
        ...
        db.query(`SELECT ps.*, st.name as signal_name, st.label as signal_label, st.unit, st.value_type
                  ...
                  WHERE ps.patient_id = $1 AND ps.site_id = $2 ORDER BY ps.collected_at DESC`, [sc.patient_id, req.user.site_id]),
        db.query(`SELECT tsr.*, st.name as signal_name, st.label as signal_label, st.unit
                  ...
                  WHERE tsr.trial_id = $1 AND tsr.is_active = true`, [sc.trial_id]),
    ]);
    res.json({ ...sc, pending_items: pendingItems.rows, signals: signals.rows, trial_signal_rules: rules.rows });
});
router.patch('/:id', async (req, res) => {
    const db = req.app.locals.db;
    const existing = (await db.query('SELECT * FROM screening_cases WHERE id = $1 AND site_id = $2', [req.params.id, req.user.site_id])).rows[0];
    if (!existing) {
        res.status(404).json({ error: 'Screening case not found' });
        return;
    }
    ...
    await (0, auth_1.auditLog)(db, { siteId: req.user.site_id, userId: req.user.id, entityType: 'screening_case', entityId: req.params.id, action: 'UPDATE', diff: req.body });
    const updated = (await db.query(`SELECT sc.*, p.first_name, p.last_name, t.name as trial_name
         FROM screening_cases sc
         JOIN patients p ON sc.patient_id = p.id
         JOIN trials t ON sc.trial_id = t.id
         WHERE sc.id = $1`, [req.params.id])).rows[0];
    res.json(updated);
});
router.post('/:id/assign', async (req, res) => {
    ...
    const result = await db.query(`UPDATE screening_cases SET assigned_user_id = $1, updated_at = NOW(), last_touched_at = NOW() WHERE id = $2 AND site_id = $3`, [user_id, req.params.id, req.user.site_id]);
    if (result.rowCount === 0) {
        res.status(404).json({ error: 'Screening case not found' });
        return;
    }
    await (0, auth_1.auditLog)(db, { siteId: req.user.site_id, userId: req.user.id, entityType: 'screening_case', entityId: req.params.id, action: 'UPDATE', diff: { assigned_user_id: user_id } });
