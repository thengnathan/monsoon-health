...
router.get('/', async (req, res) => {
    ...
    const { rows } = await db.query('SELECT * FROM signal_types WHERE site_id = $1 ORDER BY label', [req.user.site_id]);
    res.json(rows);
});
router.post('/', async (req, res) => {
    ...
    if (!name || !label || !value_type) {
        res.status(400).json({ error: 'name, label, and value_type are required' });
        return;
    }
    await db.query(`INSERT INTO signal_types (id, site_id, name, label, value_type, unit) VALUES ($1, $2, $3, $4, $5, $6)`, [id, req.user.site_id, name, label, value_type, unit || null]);
    const st = (await db.query('SELECT * FROM signal_types WHERE id = $1', [id])).rows[0];
    res.status(201).json(st);
});
router.patch('/:id', async (req, res) => {
    const db = req.app.locals.db;
    const existing = (await db.query('SELECT * FROM signal_types WHERE id = $1 AND site_id = $2', [req.params.id, req.user.site_id])).rows[0];
    if (!existing) {
