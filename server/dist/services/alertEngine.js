...
async function evaluateThresholds(db, { siteId, patientId, signalTypeId, value, signalType }) {
    const alerts = [];
    const { rows: rules } = await db.query(`SELECT tsr.*, t.name as trial_name, t.id as trial_id, t.recruiting_status
         FROM trial_signal_rules tsr
         JOIN trials t ON tsr.trial_id = t.id
         WHERE tsr.signal_type_id = $1
           AND tsr.site_id = $2
           AND tsr.is_active = true
           AND t.recruiting_status = 'ACTIVE'`, [signalTypeId, siteId]);
    for (const rule of rules) {
        if (!evaluateRule(rule, value, signalType))
            continue;
        const existingCase = (await db.query(`SELECT id, status FROM screening_cases
             WHERE patient_id = $1 AND trial_id = $2 AND site_id = $3
             AND status NOT IN ('SCREEN_FAILED', 'DECLINED', 'LOST_TO_FOLLOWUP')`, [patientId, rule.trial_id, siteId])).rows[0];
        if (existingCase && existingCase.status === 'ENROLLED')
            continue;
        ...
        try {
            ...
            await db.query(`INSERT INTO notification_events (id, site_id, type, patient_id, screening_case_id, payload, dedup_key)
                 VALUES ($1, $2, 'THRESHOLD_CROSSED', $3, $4, $5, $6)`, [eventId, siteId, patientId,
            ...
            const err = e;
            if (err.code !== '23505')
                throw e; // ignore dedup violations
        }
    }
    return alerts;
}
